...
def raycast_down(mesh_fn, x, z, default=0.0):
    """First downward hit Y at world (x, z), or `default` if the ray
    misses the mesh entirely (off the edge of the terrain)."""
    src = om2.MFloatPoint(x, RAY_HEIGHT, z)
    direction = om2.MFloatVector(0.0, -1.0, 0.0)
    hit = mesh_fn.closestIntersection(
        src, direction, om2.MSpace.kWorld, 1.0e9, False)
    if not hit or hit[2] == -1:
        return default
    return hit[0].y
...
def prepare_for_raycast(prefixes=WHEEL_PREFIXES):
...
def sample_footprints(mesh_fn, prefixes=WHEEL_PREFIXES,
                      default_ground=0.0, disconnect=False):
...
def has_footprints():
    """True if the vehicle has the raycast-able footprint groups."""
    return cmds.objExists("LF_footCenter_GRP")
...
def clear_bake(prefixes=WHEEL_PREFIXES, reconnect_flat=True):
